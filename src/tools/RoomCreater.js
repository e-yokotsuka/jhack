...
const createRoom = (inRect, retry = 0) => {
  const cell = Math.floor(mapRandom() * cellNames.length);
  ...
  const addX = Math.round(mapRandom() * ROOM_SPAN) + 2;
  const addY = Math.round(mapRandom() * ROOM_SPAN) + 2;
  const subWidth = Math.round((0.8 + mapRandom() / 5) * inRect.width) - 4;
  const subHeight = Math.round((0.8 + mapRandom() / 5) * inRect.height) - 4;
  const room = {
    x: inRect.x + addX,
    y: inRect.y + addY,
    width: subWidth - addX,
